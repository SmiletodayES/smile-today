...
Copy `.env.example` for the full list. You'll set these in Vercel (Project
...
1. Push this folder to a GitHub repo (or run `vercel` from inside it with
   the Vercel CLI — either works).
2. In Vercel: New Project → import the repo → it will auto-detect the
   `api/` functions and serve the static files with no build step needed.
3. Add the four environment variables above before the first deploy (or
   redeploy after adding them).
4. Deploy. You'll get a `*.vercel.app` URL to test with first.

## 5. Connect smiletoday.es

In the Vercel project → Settings → Domains → add `smiletoday.es` (and
`www.smiletoday.es` if you want both). Vercel will show you the DNS
records to add at your domain registrar — same kind of A/CNAME record
setup you saw in the screenshot from the other builder. Once DNS
propagates (up to 48h, usually much faster), the domain points at this
project instead.

Then go back to Stripe's webhook endpoint (step 2.3) and make sure the URL
uses the real domain, not the temporary `*.vercel.app` one.
...
2. Check Vercel's function logs (Project → Deployments → the deployment →
   Functions) to confirm `/api/webhook` ran and didn't error.
